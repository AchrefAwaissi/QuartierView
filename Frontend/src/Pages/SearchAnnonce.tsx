...
const SearchAnnonce = () => {
  ...
  const [isMaisonChecked, setIsMaisonChecked] = useState(true);
  const [isAppartementChecked, setIsAppartementChecked] = useState(true); 
  ...
  const handleSearch = async () => {
    try {
      ...
      if (ville || prixMax || isMaisonChecked || isAppartementChecked) {
        if (ville) {
          const villeRegex = new RegExp(ville, 'i');
          filteredResults = filteredResults.filter((logement: any) => villeRegex.test(logement.address));
        }
